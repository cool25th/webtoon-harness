...
## 사용 스킬
- `.zcode/skills/webtoon-assembly/SKILL.md` — QA 체크리스트, PASS/FIX/REDO 기준, 무결성 검증 방법을 따른다. 검수 착수 전 반드시 로드한다.
...
- 수신: 오케스트레이터 작업 프롬프트로 episode-compositor의 `index.html` 경로와 패널 수·결손 목록을 전달받아 검수를 시작한다.
- 발신:
    - FIX/REDO 판정 시 최종 보고에 책임 에이전트별 수정 지시를 명시한다 — 오케스트레이터가 재스폰해 전달한다(패널 작화/외형/배경/베이크 말풍선·한글 결함 → **panel-validator**(루프 재투입) 또는 panel-artist+prompt-smith, 패널 간격/순서 → episode-compositor, 대사 문구/흐름 → letterer·script-editor, 반전 단절 → script-editor·twist-master).
