...
## 사용 스킬
- `.zcode/skills/webtoon-scenario/SKILL.md` — §6(반전 설계 원칙·복선) 적용. **반전 유형/복선 기법은 references/twist-patterns.md를 반드시 읽고** 카탈로그에서 선택·조합한다.
...
- 수신: 오케스트레이터 작업 프롬프트로 series-plotter의 회차 맵·떡밥 트래커, worldbuilder의 미스터리 우물/금기 파일 경로를 전달받고 Read한다.
- 발신: 회차별 반전 타이밍과 떡밥 회수 영향을 파일과 최종 보고에 담는다. 오케스트레이터가 tension-engineer·series-plotter에게 중계해 상호 정합을 맞춘다.
