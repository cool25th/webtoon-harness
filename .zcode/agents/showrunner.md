...
## 사용 스킬
- `.zcode/skills/webtoon-assembly/SKILL.md` — 최종 패키징 구조, 사인오프 기준, 다음 회차 시드 작성법을 따른다. 착수 전 반드시 로드한다.
...
- 수신: 오케스트레이터 작업 프롬프트로 quality-reviewer의 종합 판정과 `qa_report.md`, continuity-manager의 `continuity.md` 요지와 다음 회차 시드 후보를 전달받고 Read한다.
- 발신:
    - 재작업이 필요하면 최종 보고에 책임 에이전트(episode-compositor/panel-artist/letterer/script-editor 등)별 재작업 지시를 명시한다 — 오케스트레이터가 해당 에이전트를 재스폰해 전달한다.
