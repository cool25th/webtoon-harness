...
## 사용 스킬
- `.zcode/skills/webtoon-scenario/SKILL.md` — §4(시리즈 아크 + 회차 맵) 적용. 트렌드 페이싱은 trend-brief.md 참조.
...
- 수신: 오케스트레이터 작업 프롬프트로 concept-architect/worldbuilder/character-designer의 확정 산출물 파일 경로를 전달받고 Read한다.
- 발신: 회차 맵과 떡밥 트래커, 회차별 사건 강도를 파일로 기록하고 최종 보고로 경로를 알린다. 오케스트레이터가 twist-master·tension-engineer에게 전달해(필요시 병렬 스폰) 반전·긴장 슬롯 정합을 맞춘다.
