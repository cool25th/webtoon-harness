...
## 스토리북 게이트 선행 (v10.3 — 렌더 발주 전제)

렌더 발주는 **스토리북 게이트 사용자 승인 이후에만** 진행한다(orchestrator SKILL Phase 3-6). 승인되지 않은 스크립트로 패널을 렌더하지 않는다 — 서사 결함은 렌더 전에 잡는 것이 원칙(ep01 실측: 스토리·대사 문제가 렌더 후 발견되어 전면 재작업).

## 3테이크 선택 게이트 (v10.3 — 2026-09-23 사용자 제안 채택)

- **절차**: ①매니페스트의 각 항목을 3테이크로 복제(`::panel_NNN_take1.png` ~ take3 — 동일 프롬프트, 렌더러 비결정성으로 결과 분화) ②렌더 후 `.zcode/skills/webtoon-panel-render/scripts/make_selection_page.py`로 청크 단위 비교 페이지 생성 ③사용자 선택 → 선택본을 `panel_NNN.png`로 복사 승격 ④"전부 아니야"면 해당 컷 프롬프트 방향을 사용자와 협의해 재롤.
- **명명 규칙**: 테이크 파일은 반드시 고유 파일명(takeN) — `::파일명` 치환 누락 시 DUP 핸들러가 기존 핀 파일을 삭제하는 사고 재발 방지(ep01 실측).
- **승격 전 백업**: 기존 panel_NNN.png가 있으면 반드시 `.render_logs/`로 복사 후 덮어쓴다(PR-02).

## S-1 배경 플레이트 2변형 체계 (v10.2 — 2026-09-23, 독자 실측 기반)

- **변형 2종/카메라 포지션**: `PLATE_*(EMPTY)` = 무인 무대 · `PLATE_*_FULL` = 이야기 최대 점유(제네릭 비발화 인원: 동일 평범 복장·신원 토큰 0·식별 표식 0·얼굴 디테일 없음/후면·아웃포커스). 이야기가 지정한 공석+그 마커(예: 23번 무물건 책상·반빠진 의자)는 FULL에도 유지.
- **배분 규칙**: 무인·소인원 컷→EMPTY / 군집·만석 대사 컷→**FULL**(점유율 계약: 만석 주장 대사 ≥80%·군집 배경 ≥60% — BL-22). 참조 우선 원칙 때문에 빈 참조는 텍스트의 만석 지시를 이긴다 — **점유는 참조로, 하늘·축은 텍스트 토큰으로** 담당 분리.
- **플레이트 게이트(패널 발주 전 필수)**: BL-18 그리드 기하(열 기울기 편차 ≤3°·횡오프셋 ≤15%)·BL-19 간격(통로 ≥책상 폭 60%)·BL-20 착석 포즈·BL-22 점유율·BL-23 착석 방향(전방 고정). 패널에서 이 계열 결함 발견 시 **플레이트 재렌더 우선**(패널만 고치면 상속으로 재발).
- **손 컷 원해상도 규칙**: 손 군집 컷의 손-소유자 연결 판정(BL-11-CLS)은 반해상도 컨택트시트가 아니라 **원해상도 열람**으로만 유효(실측: 컨택트시트가 유령 손 놓침).
...
| ③ 메인 | V-게이트 | 컨택트시트(2×2, 4컷 그리드) **전수 시각 열람** — 배경 일관성(플레이트 대비)·역할 확인(복장 식별)·상호작용(손-물건-사람)·화면 좌표 일관성 + **v10.2: 군집 점유율(대사 정합)·착석 포즈·착석 방향·그리드 직교성(손 컷은 원해상드 열람)** | validation.md V-게이트 섹션 |
