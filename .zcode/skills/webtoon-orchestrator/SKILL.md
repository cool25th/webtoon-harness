...
메인 에이전트(너)가 지휘자다. 팀원은 `Agent` 도구로 스폰하는 **서브에이전트**(`subagent_type: "general-purpose"`)다. 세션에 상시 팀이 존재하지 않고, 각 단계에서 필요한 역할을 그때 스폰해 산출물을 받고 정리한다. 이전 단계의 산출물은 `_workspace/`에 남으므로 다음 서브에이전트가 Read로 이어받는다.
