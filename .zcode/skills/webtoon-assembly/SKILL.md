...
### 모션 지정 (스타일별 뷰어 모션)
왜: 정지 패널에 그림체에 맞는 미세한 움직임을 얹어 "살아있는" 회차로 만든다. 과하면 베이크 텍스트 가독·구도 신뢰를 해치므로 **절제가 원칙**이다.

템플릿의 모션 체계: `<body data-style="스타일키">`로 전역 지정, 패널 항목의 `motion` 필드로 컷별 오버라이드. 값은 `생략/"auto"`(전역 따름) · 스타일 키 6종 · `"none"`(모션 없음 고정). 렌더러가 유효 키만 패널 `data-m`으로 심는다(무효키·미지정 = 모션 없음).

| 스타일 키 | 모션 |
|---|---|
| `webtoon-clean` | 미세 Ken Burns — 1.02배 초슬로우 줌, 숨쉬듯(절제) |
| `watercolor-soft` | 부드러운 드리프트 + 살짝 블러 펄스(수채 번짐 감각) |
| `retro-manga` | 슬랫(가로선) 슬라이드 등장 전환 + 톤 깜빡임 |
| `sketch-pencil` | 라인 보일 — 2프레임 지터 왕복(살아있는 스케치) |
| `noir-neon` | 네온 글로우 펄스 + 절제된 강대비 플리커 |
| `pastel-warm` | 파동치는 스케일 + 따뜻한(sepia) 페이드 |

운용 규칙:
- **플립·동일 구도 쌍은 `motion:"none"` 고정** — 구도 동일성이 반전 증명인 컷은 움직이면 증명이 깨진다(ep01 C46·C49 사례). 특수 컷도 컷별 오버라이드로 명시한다(ep01 C50 슬릿 컷 = 미세 줌만).
- 모션은 **프레임당 1종**, transform/opacity 중심(수채 블러·네온 글로우 등 스타일이 요구할 때만 최소 `filter`). JS 애니메이션 라이브러리 금지 — 단일 HTML 외부 의존성 없음 원칙 준수.
- `prefers-reduced-motion: reduce`에서는 전부 정지(CSS가 자동 처리 — 슬랫 오버레이도 제거).
- IntersectionObserver로 **화면에 들어온 패널만 재생**(`.live` 토글, 화면 밖은 정지) — 50+ 패널 회차의 렌더 부담 방지.
- 확대·이동 진폭은 베이크 말풍선 가독에 지장 없는 수준(스케일 ±2~3%·이동 0.3% 이내). 패널은 `overflow:hidden`으로 모션 여백 노출을 막는다.
...
- **KPI 원장 기입 의무(v11)**: RELEASE 패키징 시 quality-reviewer가 품질 KPI(1차 통과율·최종 승인률·결함 탈출률·패널당 재작업·BL 재발률)를 계산해 `06_assembly/metrics_ledger.md`에 측정 조건(백엔드·버전·컷 수·검증 축 세트·스타일)과 함께 1행 추가한다 — 조건이 다른 판 간 숫자 직접 비교 금지(continuity §7-33). 재작업 지표는 렌더 원장 `.render_logs/_ledger.tsv`의 attempt 열에서 산출한다.
