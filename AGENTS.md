...
- 2026-09-24: **v10.4 — 스토리북 게이트·3테이크 선택 게이트·배경 먼저·클릭 지적 게이트** (《그 자리는 비워 둬》 ep01 사용자 실측 기반). ①**스토리북 게이트**: script-editor 완료 후 `make_storybook.py`(scenario/scripts)로 회차 전체 스토리북 HTML(컷 이미지+장면+대형 말풍선+컷메타+태그) 생성 → **사용자 승인 전 렌더 금지**. 옵션: `--cast`(캐릭터 셋 최상단)·`--story-file`(스토리 요약 선행)·`--benchmarks`(유사 인기 웹툰 칩)·`--changed`(수정 컷 배지)·`--max-width`(인라인 경량화 — 49MB→1.5MB 실측)·`--feedback`(지적 UI 주입). 페이지 순서: 캐릭터 셋 → 스토리 요약 → 벤치마크 → ACT 청크별 컷(모두 details 접기). ②**3테이크 선택 게이트**: 패널당 동일 프롬프트 3테이크 → `make_selection_page.py`(panel-render/scripts) 비교 페이지 → 사용자 선택 → 승격, "전부 아니야" 재롤 포함. ③**배경 먼저 원칙**: 스토리북 승인 후 패널 렌더 전에 배경 플레이트(EMPTY/FULL×뷰)를 먼저 제작·사용자 승인 — 승인 배경이 기하 SSOT. ④**클릭 지적 게이트**: `feedback_server.py`(정적+POST /api/feedback → feedback.json JSONL, panel-render/scripts) + `feedback_snippet.html`(패널 클릭→대형 오버레이→드로잉 펜/점으로 결함 표시→태그·심각도·메모→strokes/bbox JSON 저장). ⑤**결함 원장 확장**: BL-17(문장부호 완전 베이크)·BL-18~23(그리드 기하·간격·착석 포즈·대화 스테이징·군집 점유율·착석 방향 + 교실 변형·패턴 스테이징 확장)·N-축(서사 단조성 — scenario SKILL 게이트)·PR-01~04(비전 모델 환각 좌표 PIL 금지·매 시도 보존·플레이트 게이트 전 항목 의무화·그리드 판정 사용자 선택 게이트). 근거 실측: ep01 검증 9라운드·사용자 실측 피드백 8차 — "그 자리는 비워 둬" continuity §7 연동.
